...
import { HandoffChallengeSchema } from "@/shared/contracts/src";
import { ledger } from "@/app/api/_shared/ledger";
import { getAssetIdForCustodyToken, registerHandoffChallenge } from "@/app/api/_shared/transitStore";

export async function POST(req: NextRequest) {
    try {
        const body = await req.json();
        const parsed = HandoffChallengeSchema.parse(body);
        ...
        registerHandoffChallenge(parsed.challenge_id, parsed.custody_token_id);

        const event = {
            type: "TRANSIT_HANDOFF_CHALLENGED" as const,
            asset_id,
